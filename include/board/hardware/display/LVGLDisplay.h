...
public:
    ...
    void init(
        void (*flush_cb)(lv_display_t * lvDisp, const lv_area_t * area, lv_color_t * color_p)
    ) {
        disp->init();
    ...
    void flush(lv_display_t * lvDisp, const lv_area_t *area, lv_color_t *color_p)
    {
        disp->flush(area->x1, area->y1, area->x2, area->y2, color_p);
        lv_disp_flush_ready(lvDisp);
    }
};
